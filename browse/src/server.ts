/**
 * yocode browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.yocode/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.yocode/browse-{console,network,dialog}.log
...
function generateHelpText(): string {
  ...
  const lines = ['yocode browse — headless browser for AI agents', '', 'Commands:'];
...
const SESSIONS_DIR = path.join(process.env.HOME || '/tmp', '.yocode', 'sidebar-sessions');
...
function findBrowseBin(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '..', '..', '.claude', 'skills', 'yocode', 'browse', 'dist', 'browse'),
    path.join(process.env.HOME || '', '.claude', 'skills', 'yocode', 'browse', 'dist', 'browse'),
  ];
...
function shortenPath(str: string): string {
  return str
    ...
    .replace(/\.claude\/skills\/yocode\//g, '')
...
function createWorktree(sessionId: string): string | null {
  try {
    ...
    const worktreeDir = path.join(process.env.HOME || '/tmp', '.yocode', 'worktrees', sessionId.slice(0, 8));
...
function spawnClaude(userMessage: string, extensionUrl?: string | null, forTabId?: number | null): void {
  ...
  const agentQueue = process.env.SIDEBAR_QUEUE_PATH || path.join(process.env.HOME || '/tmp', '.yocode', 'sidebar-agent-queue.jsonl');
  const yocodeDir = path.dirname(agentQueue);
  ...
  try {
    fs.mkdirSync(yocodeDir, { recursive: true });
...
async function shutdown() {
  ...
  const profileDir = path.join(process.env.HOME || '/tmp', '.yocode', 'chromium-profile');
