...
export class BrowserManager {
  ...
  /**
   * Find the yocode Chrome extension directory.
  ...
  private findExtensionPath(): string | null {
    ...
    const candidates = [
      ...
      // Global yocode install
      path.join(process.env.HOME || '', '.claude', 'skills', 'yocode', 'extension'),
      // Git repo root (detected via BROWSE_STATE_FILE location)
      (() => {
        const stateFile = process.env.BROWSE_STATE_FILE || '';
        if (stateFile) {
          const repoRoot = path.resolve(path.dirname(stateFile), '..');
          return path.join(repoRoot, '.claude', 'skills', 'yocode', 'extension');
        }
        return '';
      })(),
    ].filter(Boolean);
  ...
  async launch() {
    ...
    this.browser.on('disconnected', () => {
      ...
      console.error('[browse] Console/network logs flushed to .yocode/browse-*.log');
      process.exit(1);
    });
  ...
  // ─── Headed Mode ─────────────────────────────────────────────
  /**
   * Launch Playwright's bundled Chromium in headed mode with the yocode
  ...
  async launchHeaded(authToken?: string): Promise<void> {
    ...
    // Find the yocode extension directory for auto-loading
    ...
    const fs = require('fs');
    const path = require('path');
    const userDataDir = path.join(process.env.HOME || '/tmp', '.yocode', 'chromium-profile');
    ...
    const indicatorScript = () => {
      const injectIndicator = () => {
        if (document.getElementById('yocode-ctrl')) return;

        const topLine = document.createElement('div');
        topLine.id = 'yocode-ctrl';
        topLine.style.cssText = `
          position: fixed; top: 0; left: 0; right: 0; height: 2px;
          background: linear-gradient(90deg, #F59E0B, #FBBF24, #F59E0B);
          background-size: 200% 100%;
          animation: yocode-shimmer 3s linear infinite;
        ...
        style.textContent = `
          @keyframes yocode-shimmer {
            0% { background-position: 200% 0; }
            100% { background-position: -200% 0; }
          }
          @media (prefers-reduced-motion: reduce) {
            #yocode-ctrl { animation: none !important; }
          }
        `;
